...
import {calculateInvoiceTotalCommon,columnHeader,staticField} from "./docket.utility";
...
export class EwayBillDocketBookingV2Component implements OnInit {
  ...
  InvoiveTableEdit: boolean = false;
  InvoiveTableEditData: any;
  EditGstTable: any;
  // menuItems = [{ label: "Edit" }, { label: "Remove" }];
  // menuItemflag = true;
  // columnHeader=columnHeader;
  ...
  // metaData = {
  //   checkBoxRequired: true,
  //   noColumnSort: Object.keys(columnHeader),
  // };
  expanded = false;
  ...
  jsonControlCnoteArray: any;
  jsonControlConsignorArray:any;
  jsonControlConsigneeArray:any;

  contractData: any;
  jsonControlInvoiceArray:any;
  sampleDropdownData: any[];
  protected _onDestroy = new Subject<void>();
  tabForm: UntypedFormGroup;
  tabInvoiceForm:UntypedFormGroup;
  tableLode: boolean = true;
  tabConsignor: UntypedFormGroup;
  ...
  ngOnInit(): void {
    ...
    this.backPath = "/dashboard/Index?tab=0"
    this.expanded = true;
  }
  ...
  initializeFormControl() {
    ...
    this.jsonControlInvoiceArray=this.ewayBillTab.getInvoiceControls();
    ...
    this.tabData = {
      ...
      // "Invoice": this.jsonControlInvoiceArray
    };
    ...
    this.tabInvoiceForm = formGroupBuilder(this.fb, Object.values(this.tabInvoiceData));
  ...
  initializeGSTFormControl() {
    const customerFormControls = new EwayBillControls(
      // this.EditGstTable,
      // this.isGstUpdate
    );
    // this.jsonControlCustomerArray = customerFormControls.getFormControls();
    this.jsonControlInvoiceArray = customerFormControls.getInvoiceControls();
  ...
  calculateValidation() {
    const chargedWeight = parseFloat(
      this.tabInvoiceForm.controls["CUB_WT"]?.value || 0
    );
    const actualWeight = parseFloat(
      this.tabInvoiceForm.controls["ACT_WT"]?.value || 0
    );
  ...
  async AddRowData() {
    ...
    // const Index = this.InvoiveTableEdit ? this.InvoiveTableEditData.INVNO :
    // this.tableData.length == 0 ? 1 : this.tableData.slice(-1)[0].INVNO + 1;
    if (await this.calculateValidation()) 
    {
      const invoice = this.tableData;      
      if (invoice.length > 0) {
        const exist = invoice.find(
          (x) => x.INVNO === this.tabInvoiceForm.value.INVNO
        );
        if (exist) {
          this.tabInvoiceForm.controls["INVNO"].setValue("");
          Swal.fire({
...
// Assuming gmtDate is the GMT date string you want to format
const gmtDate = this.tabInvoiceForm.value.INVDT;

// Convert GMT date string to a JavaScript Date object
const dateObject = new Date(gmtDate);

// Format the date using moment.js
const formattedDate = moment.utc(dateObject).format('DD-MM-YYYY');

    const json = {
      // Srno: parseInt(Index),
      INVNO:this.tabInvoiceForm.value.INVNO,
      INVDT:formattedDate,
      // INVDT:this.tabInvoiceForm.value.INVDT,
      LENGTH:this.tabInvoiceForm.value.LENGTH,
      BREADTH:this.tabInvoiceForm.value.BREADTH,
      HEIGHT:this.tabInvoiceForm.value.HEIGHT,
      DECLVAL:this.tabInvoiceForm.value.DECLVAL,
      NO_PKGS:this.tabInvoiceForm.value.NO_PKGS,
      CUB_WT:this.tabInvoiceForm.value.CUB_WT,
      ACT_WT:this.tabInvoiceForm.value.ACT_WT,
      Invoice_Product:this.tabInvoiceForm.value.Invoice_Product,
      HSN_CODE:this.tabInvoiceForm.value.HSN_CODE,
      actions: ["Edit", "Remove"],
  ...
  //  this.calculateInvoiceTotal();
    ...
    this.tabInvoiceForm.reset();
    // const datajason=this.tableData;
  ...
  addRemoveGSTValue(data, type) {
    ...
    const invoiceDateStr = data?.INVDT; // Assuming INVDT is the date field
    if (invoiceDateStr) {
      const parts = invoiceDateStr.split('-');
      const day = parseInt(parts[0], 10);
      const month = parseInt(parts[1], 10) - 1; // Month in JavaScript Date starts from 0
      const year = parseInt(parts[2], 10); // Assuming years are in the 21st century
      const date = new Date(year, month, day);
      
      this.tabInvoiceForm.controls["INVDT"].setValue(date);
    }
    ...
    this.tabInvoiceForm.controls["INVNO"].setValue(
      this.InvoiveTableEdit ? data?.INVNO : ""
    );
    // this.tabInvoiceForm.controls["INVDT"].setValue(
    //   this.InvoiveTableEdit ? data?.INVDT : ""
    // );
    this.tabInvoiceForm.controls["LENGTH"].setValue(
      this.InvoiveTableEdit ? data?.LENGTH : ""
    );
    this.tabInvoiceForm.controls["BREADTH"].setValue(
      this.InvoiveTableEdit ? data?.BREADTH : ""
    );
    this.tabInvoiceForm.controls["HEIGHT"].setValue(
      this.InvoiveTableEdit ? data?.HEIGHT : ""
    );
    this.tabInvoiceForm.controls["DECLVAL"].setValue(
      this.InvoiveTableEdit ? data?.DECLVAL : ""
    );
    this.tabInvoiceForm.controls["NO_PKGS"].setValue(
      this.InvoiveTableEdit ? data?.NO_PKGS : ""
    );
    this.tabInvoiceForm.controls["NO_PKGS"].setValue(
      this.InvoiveTableEdit ? data?.NO_PKGS : ""
    );
    this.tabInvoiceForm.controls["CUB_WT"].setValue(
      this.InvoiveTableEdit ? data?.CUB_WT : ""
    );
    this.tabInvoiceForm.controls["ACT_WT"].setValue(
      this.InvoiveTableEdit ? data?.ACT_WT : ""
    );
    this.tabInvoiceForm.controls["Invoice_Product"].setValue(
      this.InvoiveTableEdit ? data?.Invoice_Product : ""
    );
    this.tabInvoiceForm.controls["HSN_CODE"].setValue(
      this.InvoiveTableEdit ? data?.HSN_CODE : ""
    );
    // this.delete(event);

    if (!this.InvoiveTableEdit) {
  ...
  async handleMenuItemClick(data) {
    ...
    if (data.label.label == "Edit") {
      ...
      this.InvoiveTableEdit = true;
      this.InvoiveTableEditData = data.data;
      // console.log("hello");
      console.log("InvoiveTableEditData",this.InvoiveTableEditData);
  ...
  async saveData() {
    ...
    const tabInvoiceForm = this.tabInvoiceForm;
    clearValidatorsAndValidate(tabInvoiceForm);
    
    console.log("tabcontrols",tabcontrols);
    console.log("tabInvoiceForm",tabInvoiceForm);
    console.log("contractcontrols",contractcontrols);

    this.tableData = this.tableData.map(item => {
      return { ...item, _id: item.INVNO };
    });
    const invoiceData = this.tableData.map(x => {
      const { actions, id, ...rest } = x;
      return rest;
    });
    console.log("INVOICE DATA",invoiceData);
    // clearValidatorsAndValidate(invoicecontrols);
  ...
  async checkInvoiceExist(){
    ...
    if(this.tabInvoiceForm.value.INVNO){
    const res=await this.docketService.checkInvoiceExistLTL({cID:this.storage.companyCode,iNVNO:this.tabInvoiceForm.value.INVNO});
    if(res){
      Swal.fire({
        icon: 'info',
        title: 'Invoice Exists',
        text: `Invoice number ${this.tabInvoiceForm.value.INVNO} already exists.`
      });
      this.tabInvoiceForm.value.INVNO = '';
    }
    else{
      const existingInvoice = this.tableData.filter((item) => item.INVNO == this.tabInvoiceForm.value.INVNO);
